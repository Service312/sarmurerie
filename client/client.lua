        ...
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuArmurerie, function()
                    ...
                    RageUI.Separator("  ↓                         ~p~Accessoire                        ~s~↓")
                    for k, v in pairs(Config.accessoire) do 
                        RageUI.Button(v.Nom .. '', nil, {RightLabel = "~p~" ..v.Price.." $"}, true, {
                            onSelected = function()
                                TriggerServerEvent('serv:Arme', v.Nom, v.Arme, v.Price)
                                RageUI.CloseAll()
                            end
                        })
                    end 
    ...
    while true do
		local wait = 750
            ...
            if dist <= Config.MarkerDistance then
                wait = 0
                ESX.ShowHelpNotification("Appuyer sur [~p~E~s~] pour accéder à ~p~l'armurerie !")
...
RegisterNetEvent('serv:giveArme')
AddEventHandler('serv:giveArme', function(gun)
